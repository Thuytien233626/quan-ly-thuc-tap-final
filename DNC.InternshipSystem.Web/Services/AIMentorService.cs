...
namespace DNC.InternshipSystem.Web.Services
{
    public class AIMentorService
    {
        ...
        {
            var endpoint = $"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-pro:generateContent?key={apiKey}";
            ...
            var requestBody = new
            {
                ...
                {
                    parts = new[] {
                        new { text = "Bạn là AI Mentor chuyên nghiệp, phong cách làm việc chuyên nghiệp giàu kinh nghiệm, đánh giá đúng trọng tâm, tư duy hiện đại. Hãy phân tích logbook và trả về DUY NHẤT JSON với CÁC TÊN KEY CHÍNH XÁC SAU: \"AISummary\" (chuỗi, tóm tắt 2 câu), \"AiScore\" (số nguyên 0-100), \"AIWarning\" (boolean, true nếu làm sơ sài), \"AIWarningDetails\" (chuỗi, lý do cảnh báo, rỗng nếu không có), \"AiSuggestions\" (chuỗi, 2 gạch đầu dòng gợi ý)." }
                    }
                },
                contents = new[]
                {
